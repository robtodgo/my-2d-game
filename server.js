...
// Аккаунты (в памяти)
const accounts = {}; // nickname -> { password, color, avatar }
...
io.on('connection', (socket) => {
    ...
    socket.on('attack', (targetId) => {
        ...
        if (target.hp <= 0) {
            ...
            io.emit('chat message', { sender: '☠️', text: `${target.nickname} был убит ${attacker.nickname}` });
        }
    });
    ...
    socket.on('chat message', (msg) => {
        ...
        if (cmd === 'find' || cmd === 'profile') {
            ...
            if (target) {
                socket.emit('chat message', { sender: '📋', text: `${target.nickname}: ❤️${target.hp}/${target.maxHp} 📍${Math.round(target.x)},${Math.round(target.y)}` });
            } else {
                socket.emit('chat message', { sender: '❌', text: 'Игрок не в сети' });
            }
            ...
            if (target) {
                ...
                socket.emit('chat message', { sender: '✨', text: `Телепортирован к ${target.nickname}` });
            } else {
                socket.emit('chat message', { sender: '❌', text: 'Игрок не в сети' });
            }
        } else if (cmd === 'help') {
            socket.emit('chat message', { sender: '📚', text: '/find <ник>, /profile <ник>, /tp <ник>, /op <пароль>' });
        } else if (cmd === 'op' && args[1] === ADMIN_PASSWORD) {
            admins.add(socket.id);
            socket.emit('admin status', true);
            socket.emit('chat message', { sender: '🔓', text: 'Права администратора получены' });
            ...
            if (cmd === 'give' && args[1] && args[2]) {
                ...
                if (target) {
                    ...
                    if (added) {
                        ...
                        socket.emit('chat message', { sender: '🎁', text: `Выдано ${item.count}x ${item.name} игроку ${target.nickname}` });
                    }
                }
                ...
                socket.emit('chat message', { sender: '❓', text: 'Неизвестная админ-команда' });
            }
        } else {
            socket.emit('chat message', { sender: '❓', text: 'Неизвестная команда. /help' });
        }
    });
